...
import { createLocationPlanSchema, locationPlanIdParamSchema } from '../schemas/location-plan.schema.js';
...
export async function locationPlanRoutes(app: FastifyInstance): Promise<void> {
  const createLocationPlanUseCase = app.diContainer.resolve<CreateLocationPlanUseCase>('createLocationPlanUseCase');
  const getLocationPlanUseCase = app.diContainer.resolve<GetLocationPlanUseCase>('getLocationPlanUseCase');
  const storageService = app.diContainer.resolve<MinioStorageService>('storageService');

  app.post('/', {
    schema: { description: 'Générer un plan de localisation (QGIS)', tags: ['Plans de localisation'], security: [{ bearerAuth: [] }], body: zodToSwagger(createLocationPlanSchema) },
  ...
  app.get('/:id', {
    schema: { description: 'Obtenir le statut d’un plan de localisation', tags: ['Plans de localisation'], security: [{ bearerAuth: [] }] },
    preHandler: [app.authenticate],
  }, async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.get('/:id/download', {
    schema: { description: 'Télécharger le PDF du plan de localisation', tags: ['Plans de localisation'], security: [{ bearerAuth: [] }] },
    preHandler: [app.authenticate],
  }, async (request: FastifyRequest, reply: FastifyReply) => {
    const { id } = parseBody(locationPlanIdParamSchema, request.params);
    const result = await getLocationPlanUseCase.execute(id);
    if (!result.filePath) {
      throw new ValidationError('Location plan not ready for download', { status: result.status });
    }
