...
import {
  SaveDrawingUseCase,
  SaveDrawingDTO,
} from '../../application/use-cases/drawings/save-drawing.use-case.js';
...
export async function drawingRoutes(app: FastifyInstance): Promise<void> {
  ...
  const deleteDrawingUseCase =
    app.diContainer.resolve<DeleteDrawingUseCase>('deleteDrawingUseCase');
  ...
  app.get(
    ...
    {
      schema: {
        description: 'Lister les dessins',
        tags: ['Dessins'],
        security: [{ bearerAuth: [] }],
  ...
  app.get(
    ...
    {
      schema: {
        description: 'Obtenir un dessin par ID',
        tags: ['Dessins'],
        security: [{ bearerAuth: [] }],
      },
      preHandler: [app.authenticate],
  ...
  app.post(
    ...
    {
      schema: {
        description: 'Creer un dessin',
        tags: ['Dessins'],
        security: [{ bearerAuth: [] }],
        body: zodToSwagger(createDrawingSchema),
      },
      preHandler: [app.authenticate],
    },
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const userId = (request.user as { sub: string }).sub;
      const drawing = await saveDrawingUseCase.execute(userId, dto.instanceId, {
        ...dto,
        geojson: dto.geojson as SaveDrawingDTO['geojson'],
      });
  ...
  app.delete(
    ...
    {
      schema: {
        description: 'Supprimer un dessin',
        tags: ['Dessins'],
        security: [{ bearerAuth: [] }],
      },
      preHandler: [app.authenticate],
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { id } = parseBody(idParamSchema, request.params);
      const userId = (request.user as { sub: string }).sub;
      await deleteDrawingUseCase.execute(userId, id);
