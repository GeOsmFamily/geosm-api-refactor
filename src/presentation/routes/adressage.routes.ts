...
export async function adressageRoutes(app: FastifyInstance): Promise<void> {
  ...
  const searchAdresseUseCase = app.diContainer.resolve<SearchAdresseUseCase>('searchAdresseUseCase');
  const getAdresseByClickUseCase = app.diContainer.resolve<GetAdresseByClickUseCase>('getAdresseByClickUseCase');
  ...
  app.post('/adresse', {
    schema: { description: 'Obtenir une adresse', tags: ['Adressage'], body: zodToSwagger(getAdresseSchema) },
  ...
  app.post('/position', {
    schema: { description: 'Obtenir la position d\'une adresse', tags: ['Adressage'], body: zodToSwagger(getPositionSchema) },
  }, async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.post('/points', {
    schema: { description: 'Obtenir les points d\'une rue', tags: ['Adressage'], body: zodToSwagger(getPointsSchema) },
  }, async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.post('/search', {
    schema: { description: 'Rechercher une adresse par usage', tags: ['Adressage'], body: zodToSwagger(searchAdresseSchema) },
  }, async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.post('/click', {
    schema: { description: 'Obtenir l\'adresse par clic sur la carte', tags: ['Adressage'], body: zodToSwagger(getAdresseByClickSchema) },
  }, async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.post('/elastic-data', {
    schema: { description: 'Obtenir les donnees Elasticsearch', tags: ['Adressage'], body: zodToSwagger(getElasticDataSchema) },
  }, async (request: FastifyRequest, reply: FastifyReply) => {
