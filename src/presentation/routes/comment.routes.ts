...
export async function commentRoutes(app: FastifyInstance): Promise<void> {
  ...
  const deleteCommentUseCase =
    app.diContainer.resolve<DeleteCommentUseCase>('deleteCommentUseCase');
  const replyToCommentUseCase =
    app.diContainer.resolve<ReplyToCommentUseCase>('replyToCommentUseCase');
  const resolveCommentUseCase =
    app.diContainer.resolve<ResolveCommentUseCase>('resolveCommentUseCase');
  ...
  app.get(
    ...
    {
      schema: {
        description: 'Lister les commentaires (avec leurs réponses)',
        tags: ['Commentaires'],
        security: [{ bearerAuth: [] }],
  ...
  app.post(
    '/',
    {
      schema: {
        ...
        tags: ['Commentaires'],
        security: [{ bearerAuth: [] }],
        body: zodToSwagger(createCommentSchema),
      },
      preHandler: [app.authenticate],
    },
    async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.post(
    '/:id/reply',
    {
      schema: {
        ...
        tags: ['Commentaires'],
        security: [{ bearerAuth: [] }],
        body: zodToSwagger(replyCommentSchema),
      },
      preHandler: [app.authenticate],
    },
    async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.patch(
    ...
    {
      schema: {
        description: 'Marquer un commentaire comme résolu/non résolu',
        tags: ['Commentaires'],
        security: [{ bearerAuth: [] }],
        body: zodToSwagger(resolveCommentSchema),
      },
      preHandler: [app.authenticate],
    },
    async (request: FastifyRequest, reply: FastifyReply) => {
