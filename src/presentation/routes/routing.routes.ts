...
export async function routingRoutes(app: FastifyInstance): Promise<void> {
  const calculateRouteUseCase =
    app.diContainer.resolve<CalculateRouteUseCase>('calculateRouteUseCase');
  ...
  app.get(
    ...
    {
      schema: {
        description: 'Calculer un itineraire',
        tags: ['Itineraire'],
        querystring: zodToSwagger(routeQuerySchema),
      },
    },
  ...
  app.get(
    '/nearest',
    {
      schema: {
        ...
        tags: ['Itineraire'],
        querystring: zodToSwagger(nearestQuerySchema),
      },
    },
    async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.get(
    '/nearest-feature',
    {
      schema: {
        description:
          "Trouver les features d'une couche les plus proches d'un point (distance routière)",
        tags: ['Itineraire'],
        querystring: zodToSwagger(nearestFeatureQuerySchema),
      },
    },
    async (request: FastifyRequest, reply: FastifyReply) => {
      const query = parseBody(nearestFeatureQuerySchema, request.query);
      const result = await findNearestFeatureUseCase.execute(
        query.layerId,
        query.lon,
        query.lat,
        query.limit,
