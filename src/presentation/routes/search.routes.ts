...
import {
  ...
  searchLayersQuerySchema,
  searchFeaturesQuerySchema,
  searchSuggestionsQuerySchema,
  layerRecommendationsQuerySchema,
...
export async function searchRoutes(app: FastifyInstance): Promise<void> {
  ...
  const searchFeaturesUseCase =
    app.diContainer.resolve<SearchFeaturesUseCase>('searchFeaturesUseCase');
  const getSearchSuggestionsUseCase = app.diContainer.resolve<GetSearchSuggestionsUseCase>(
    'getSearchSuggestionsUseCase',
  );
  const getLayerRecommendationsUseCase = app.diContainer.resolve<GetLayerRecommendationsUseCase>(
    'getLayerRecommendationsUseCase',
  );
  ...
  app.get(
    '/',
    {
      schema: {
        ...
        tags: ['Recherche'],
        querystring: zodToSwagger(globalSearchQuerySchema),
      },
    },
  ...
  app.get(
    ...
    {
      schema: {
        description: 'Rechercher des couches',
        tags: ['Recherche'],
        querystring: zodToSwagger(searchLayersQuerySchema),
      },
    },
    async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.get(
    ...
    {
      schema: {
        description: 'Rechercher des entites geographiques',
        tags: ['Recherche'],
        querystring: zodToSwagger(searchFeaturesQuerySchema),
      },
    },
    async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.get(
    ...
    {
      schema: {
        description: 'Suggestions de recherche contextuelles',
        tags: ['Recherche'],
        querystring: zodToSwagger(searchSuggestionsQuerySchema),
      },
  ...
  app.get(
    ...
    {
      schema: {
        description: 'Recommandation de couches par co-occurrence',
        tags: ['Recherche'],
        querystring: zodToSwagger(layerRecommendationsQuerySchema),
      },
    },
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const acceptLang = request.headers['accept-language'];
      const lang = acceptLang ? acceptLang.split(',')[0].split('-')[0].trim().toLowerCase() : 'fr';
      const result = await getLayerRecommendationsUseCase.execute(
        query.layerId,
        query.instanceId,
        query.limit,
        lang,
