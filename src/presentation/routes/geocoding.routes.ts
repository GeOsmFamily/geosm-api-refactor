...
import {
  searchGeocodingQuerySchema,
  reverseGeocodingQuerySchema,
  lookupGeocodingQuerySchema,
} from '../schemas/geocoding.schema.js';
...
export async function geocodingRoutes(app: FastifyInstance): Promise<void> {
  const searchGeocodingUseCase =
    app.diContainer.resolve<SearchGeocodingUseCase>('searchGeocodingUseCase');
  const reverseGeocodingUseCase =
    app.diContainer.resolve<ReverseGeocodingUseCase>('reverseGeocodingUseCase');
  const lookupGeocodingUseCase =
    app.diContainer.resolve<LookupGeocodingUseCase>('lookupGeocodingUseCase');

  app.get(
    '/search',
    {
      schema: {
        description: 'Rechercher une adresse par texte',
        tags: ['Geocodage'],
        querystring: zodToSwagger(searchGeocodingQuerySchema),
      },
    },
  ...
  app.get(
    ...
    {
      schema: {
        description: 'Geocodage inverse (coordonnees vers adresse)',
        tags: ['Geocodage'],
        querystring: zodToSwagger(reverseGeocodingQuerySchema),
      },
    },
    async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.get(
    ...
    {
      schema: {
        description: 'Rechercher par identifiants OSM',
        tags: ['Geocodage'],
        querystring: zodToSwagger(lookupGeocodingQuerySchema),
      },
    },
    async (request: FastifyRequest, reply: FastifyReply) => {
