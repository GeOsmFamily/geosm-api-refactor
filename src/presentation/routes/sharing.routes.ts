...
import {
  ...
  CreateSharedMapDTO,
} from '../../application/use-cases/sharing/create-shared-map.use-case.js';
...
export async function sharingRoutes(app: FastifyInstance): Promise<void> {
  const createSharedMapUseCase =
    app.diContainer.resolve<CreateSharedMapUseCase>('createSharedMapUseCase');
  ...
  app.post(
    ...
    {
      schema: {
        description: 'Creer un partage de carte',
        tags: ['Partage'],
        security: [{ bearerAuth: [] }],
        body: zodToSwagger(createShareSchema),
      },
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const shared = await createSharedMapUseCase.execute(userId, dto.instanceId, {
        ...dto,
        mapState: dto.mapState as CreateSharedMapDTO['mapState'],
      });
