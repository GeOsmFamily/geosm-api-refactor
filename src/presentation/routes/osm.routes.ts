...
export async function osmRoutes(app: FastifyInstance): Promise<void> {
  ...
  const createOsmTableUseCase =
    app.diContainer.resolve<CreateOsmTableUseCase>('createOsmTableUseCase');
  const listOsmTagKeysUseCase =
    app.diContainer.resolve<ListOsmTagKeysUseCase>('listOsmTagKeysUseCase');
  const listOsmTagValuesUseCase =
    app.diContainer.resolve<ListOsmTagValuesUseCase>('listOsmTagValuesUseCase');
  ...
  app.get(
    ...
    {
      schema: {
        description: 'Lister les clés de tag OSM disponibles',
        tags: ['OpenStreetMap'],
        security: [{ bearerAuth: [] }],
        querystring: zodToSwagger(tagKeysQuerySchema),
      },
      preHandler: [
        app.authenticate,
  ...
  app.get(
    ...
    {
      schema: {
        description: "Lister les valeurs distinctes d'une clé de tag OSM",
        tags: ['OpenStreetMap'],
        security: [{ bearerAuth: [] }],
        querystring: zodToSwagger(tagValuesQuerySchema),
      },
      preHandler: [
        app.authenticate,
        requireRole(Role.SUPER_ADMIN, Role.ADMIN_INSTANCE, Role.EDITOR),
  ...
  app.post(
    ...
    {
      schema: {
        description: 'Interroger les donnees OSM',
        tags: ['OpenStreetMap'],
        security: [{ bearerAuth: [] }],
        body: zodToSwagger(osmQuerySchema),
      },
  ...
  app.post(
    ...
    {
      schema: {
        description: 'Creer une table derivee a partir des donnees OSM',
        tags: ['OpenStreetMap'],
        security: [{ bearerAuth: [] }],
        body: zodToSwagger(createOsmTableSchema),
      },
