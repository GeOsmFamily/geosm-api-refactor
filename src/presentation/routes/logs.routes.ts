...
export async function logsRoutes(app: FastifyInstance): Promise<void> {
  const logFrontendErrorUseCase = app.diContainer.resolve<LogFrontendErrorUseCase>('logFrontendErrorUseCase');
  ...
  app.post('/frontend-error', {
    schema: { description: 'Remonter une erreur JS non gérée côté frontend', tags: ['Logs'], body: zodToSwagger(frontendErrorSchema) },
