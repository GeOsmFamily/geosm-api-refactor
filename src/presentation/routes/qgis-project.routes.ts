...
const qgisProjectIdParamSchema = z.object({
  instanceId: z.string().uuid(),
  qgisProjectId: z.string().uuid(),
});

export async function qgisProjectRoutes(app: FastifyInstance): Promise<void> {
  const getQgisProjectUseCase =
    app.diContainer.resolve<GetQgisProjectUseCase>('getQgisProjectUseCase');
  const reloadQgisProjectUseCase = app.diContainer.resolve<ReloadQgisProjectUseCase>(
    'reloadQgisProjectUseCase',
  );
  const uploadQgisProjectUseCase = app.diContainer.resolve<UploadQgisProjectUseCase>(
    'uploadQgisProjectUseCase',
  );
  const listQgisProjectLayersUseCase = app.diContainer.resolve<ListQgisProjectLayersUseCase>(
    'listQgisProjectLayersUseCase',
  );
  const createLayersFromQgisProjectUseCase =
    app.diContainer.resolve<CreateLayersFromQgisProjectUseCase>(
  ...
  app.get(
    ...
    {
      schema: {
        description: "Obtenir le projet QGIS d'une instance",
        tags: ['Projets QGIS'],
        security: [{ bearerAuth: [] }],
      },
  ...
  app.post(
    ...
    {
      schema: {
        description: "Recharger le projet QGIS d'une instance",
        tags: ['Projets QGIS'],
        security: [{ bearerAuth: [] }],
      },
  ...
  app.post(
    ...
    {
      schema: {
        description: 'Uploader un projet QGIS (.qgz ou .zip contenant un .qgs)',
        tags: ['Projets QGIS'],
        security: [{ bearerAuth: [] }],
      },
      preHandler: [app.authenticate, requireRole(Role.SUPER_ADMIN, Role.ADMIN_INSTANCE)],
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (buffer.length > UPLOAD_MAX_FILE_SIZE) {
        throw new ValidationError(
          `File too large. Maximum size is ${UPLOAD_MAX_FILE_SIZE / 1024 / 1024}MB`,
          {},
        );
      }
  ...
  app.get(
    ...
    {
      schema: {
        description: "Lister les couches d'un projet QGIS",
        tags: ['Projets QGIS'],
        security: [{ bearerAuth: [] }],
      },
      preHandler: [app.authenticate],
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const result = await createLayersFromQgisProjectUseCase.execute(instanceId, {
        qgisProjectId,
        ...dto,
      });
      return reply.status(201).send(successResponse(result));
  ...
  app.get(
    ...
    {
      schema: {
        description: 'Télécharger un projet QGIS complet (données + styles), autonome',
        tags: ['Projets QGIS'],
        security: [{ bearerAuth: [] }],
      },
      preHandler: [app.authenticate, requireRole(Role.SUPER_ADMIN, Role.ADMIN_INSTANCE)],
