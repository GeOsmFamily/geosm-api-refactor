...
async function checkPostgres(app: FastifyInstance): Promise<HealthCheckResult> {
  ...
  try {
    const prisma = app.diContainer?.resolve('prisma') as
...
async function checkRedis(app: FastifyInstance): Promise<HealthCheckResult> {
  const start = Date.now();
  try {
    const redisService = app.diContainer?.resolve('redisService') as
...
async function checkMinio(app: FastifyInstance): Promise<HealthCheckResult> {
  const start = Date.now();
  try {
    const storageService = app.diContainer?.resolve('storageService') as
...
export async function healthRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get(
    ...
    async (_request: FastifyRequest, reply: FastifyReply) => {
      ...
      checks.redis =
        redis.status === 'fulfilled' ? redis.value : { status: 'unhealthy', error: 'Check failed' };
      checks.minio =
        minio.status === 'fulfilled' ? minio.value : { status: 'unhealthy', error: 'Check failed' };
      checks.meilisearch =
        meilisearch.status === 'fulfilled'
          ? meilisearch.value
          : { status: 'unhealthy', error: 'Check failed' };
      checks.qgisServer =
        qgis.status === 'fulfilled' ? qgis.value : { status: 'unhealthy', error: 'Check failed' };
      checks.disk = getDiskUsage();
      checks.memory = getMemoryUsage();

      // BullMQ queue status
      try {
        const queueService = app.diContainer?.resolve('queueService') as
          | {
              getQueueNames: () => string[];
              getJobCounts: (name: string) => Promise<Record<string, number>>;
            }
          | undefined;
        if (queueService) {
  ...
  app.get(
    ...
    async (_request: FastifyRequest, reply: FastifyReply) => {
      return reply.send(
        successResponse({ status: 'live', pid: process.pid, uptime: process.uptime() }),
      );
    },
  );
