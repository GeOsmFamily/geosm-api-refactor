...
export async function adminCommentRoutes(app: FastifyInstance): Promise<void> {
  const adminListCommentsUseCase = app.diContainer.resolve<AdminListCommentsUseCase>('adminListCommentsUseCase');
  const flagCommentUseCase = app.diContainer.resolve<FlagCommentUseCase>('flagCommentUseCase');
  const adminDeleteCommentUseCase = app.diContainer.resolve<AdminDeleteCommentUseCase>('adminDeleteCommentUseCase');

  app.get('/', {
    schema: { description: 'Lister les commentaires pour modération', tags: ['Administration'], security: [{ bearerAuth: [] }], querystring: zodToSwagger(listQuerySchema) },
  ...
  app.post('/:id/flag', {
    schema: { description: 'Signaler un commentaire', tags: ['Administration'], security: [{ bearerAuth: [] }], body: zodToSwagger(flagCommentSchema) },
    preHandler: [app.authenticate, requireRole(Role.SUPER_ADMIN, Role.ADMIN_INSTANCE)],
  }, async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.post('/:id/unflag', {
    schema: { description: 'Lever le signalement d\'un commentaire', tags: ['Administration'], security: [{ bearerAuth: [] }] },
    preHandler: [app.authenticate, requireRole(Role.SUPER_ADMIN, Role.ADMIN_INSTANCE)],
  }, async (request: FastifyRequest, reply: FastifyReply) => {
    const { id } = parseBody(idParamSchema, request.params);
  ...
  app.delete('/:id', {
    schema: { description: 'Supprimer un commentaire (modération)', tags: ['Administration'], security: [{ bearerAuth: [] }] },
    preHandler: [app.authenticate, requireRole(Role.SUPER_ADMIN, Role.ADMIN_INSTANCE)],
  }, async (request: FastifyRequest, reply: FastifyReply) => {
    const { id } = parseBody(idParamSchema, request.params);
