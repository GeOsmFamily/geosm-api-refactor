...
export async function geosignetRoutes(app: FastifyInstance): Promise<void> {
  const saveGeosignetUseCase = app.diContainer.resolve<SaveGeosignetUseCase>('saveGeosignetUseCase');
  const getGeosignetsUseCase = app.diContainer.resolve<GetGeosignetsUseCase>('getGeosignetsUseCase');
  const deleteGeosignetUseCase = app.diContainer.resolve<DeleteGeosignetUseCase>('deleteGeosignetUseCase');

  // GET /api/v1/geosignets
  app.get('/', {
    schema: { description: 'Lister les géosignets', tags: ['Géosignets'], security: [{ bearerAuth: [] }] },
  ...
  app.post('/', {
    schema: { description: 'Créer un géosignet', tags: ['Géosignets'], security: [{ bearerAuth: [] }], body: zodToSwagger(createGeosignetSchema) },
    preHandler: [app.authenticate],
  }, async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.delete('/:id', {
    schema: { description: 'Supprimer un géosignet', tags: ['Géosignets'], security: [{ bearerAuth: [] }] },
    preHandler: [app.authenticate],
  }, async (request: FastifyRequest, reply: FastifyReply) => {
