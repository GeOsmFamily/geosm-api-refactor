...
export async function uploadRoutes(app: FastifyInstance): Promise<void> {
  ...
  const downloadExportUseCase =
    app.diContainer.resolve<DownloadExportUseCase>('downloadExportUseCase');
  ...
  app.post(
    ...
    {
      schema: {
        description: 'Importer un fichier geospatial (multipart)',
        tags: ['Import de donnees'],
        security: [{ bearerAuth: [] }],
      },
      preHandler: [
        app.authenticate,
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (!params.success)
        throw new ValidationError(
          'Invalid layer ID',
          params.error.format() as Record<string, unknown>,
        );
      ...
      if (
        !ALLOWED_MIMETYPES.includes(data.mimetype) &&
