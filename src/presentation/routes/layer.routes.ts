...
export async function layerRoutes(app: FastifyInstance): Promise<void> {
  ...
  const stageFileImportUseCase = app.diContainer.resolve<StageFileImportUseCase>('stageFileImportUseCase');
  app.post('/import/file', {
    schema: { description: 'Importer un fichier géospatial en staging (aperçu avant publication)', tags: ['Couches'], security: [{ bearerAuth: [] }] },
    preHandler: [app.authenticate, requireRole(Role.SUPER_ADMIN, Role.ADMIN_INSTANCE, Role.EDITOR)],
  }, async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  const createLayerFromStagingUseCase = app.diContainer.resolve<CreateLayerFromStagingUseCase>('createLayerFromStagingUseCase');
  ...
  app.post('/import/file/confirm', {
    schema: { description: 'Confirmer et publier une couche importée depuis un fichier', tags: ['Couches'], security: [{ bearerAuth: [] }], body: zodToSwagger(confirmFileImportSchema) },
    preHandler: [app.authenticate, requireRole(Role.SUPER_ADMIN, Role.ADMIN_INSTANCE, Role.EDITOR)],
  }, async (request: FastifyRequest, reply: FastifyReply) => {
    const { instanceId } = parseBody(instanceIdParamSchema, request.params);
  ...
  const createLayerFromOsmUseCase = app.diContainer.resolve<CreateLayerFromOsmUseCase>('createLayerFromOsmUseCase');
  ...
  app.post('/import/osm/confirm', {
    schema: { description: 'Confirmer et publier une couche filtrée par tag OSM', tags: ['Couches'], security: [{ bearerAuth: [] }], body: zodToSwagger(confirmOsmImportSchema) },
    preHandler: [app.authenticate, requireRole(Role.SUPER_ADMIN, Role.ADMIN_INSTANCE, Role.EDITOR)],
  }, async (request: FastifyRequest, reply: FastifyReply) => {
    const { instanceId } = parseBody(instanceIdParamSchema, request.params);
  ...
  const applyLayerStyleUseCase = app.diContainer.resolve<ApplyLayerStyleUseCase>('applyLayerStyleUseCase');
  ...
  app.post('/:id/style/apply', {
    schema: { description: 'Appliquer un style (couleur+icône ou KML) à une couche', tags: ['Couches'], security: [{ bearerAuth: [] }] },
    preHandler: [app.authenticate, requireRole(Role.SUPER_ADMIN, Role.ADMIN_INSTANCE, Role.EDITOR)],
  }, async (request: FastifyRequest, reply: FastifyReply) => {
    const { id } = parseBody(layerIdParamSchema, request.params);
    ...
    if (mode === 'kml') {
      ...
      await writeFile(tmpPath, buffer);
      try {
        const result = await applyLayerStyleUseCase.execute({ layerId: id, mode: 'kml', kmlFilePath: tmpPath });
        return reply.send(successResponse(result));
      } finally {
        await unlink(tmpPath).catch(() => undefined);
      }
    ...
    const result = await applyLayerStyleUseCase.execute({ layerId: id, mode: 'color-icon', color, iconKey, shape });
    return reply.send(successResponse(result));
  });
}
