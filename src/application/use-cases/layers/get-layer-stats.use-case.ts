...
const NARRATIVE_PROMPT_BY_LANG: Record<string, (layerName: string, stats: LayerStats) => string> = {
  fr: (layerName, stats) => `Décris en une phrase concise et naturelle (en français, pour un géoportail public, sans jargon) `
  ...
  en: (layerName, stats) => `Describe in one concise, natural sentence (in English, for a public geoportal, no jargon) `
...
export class GetLayerStatsUseCase {
  ...
  async execute(layerId: string, withNarrative = false, lang = 'fr'): Promise<LayerStatsWithNarrative> {
    ...
    try {
      ...
      const narrative = await this.geminiService.generateText(buildPrompt(localize(layer.name, lang), stats));
