...
import {
  defaultCategories,
  defaultLayers,
} from '../../../domain/constants/default-layers.constants.js';
...
const LAYER_SVG_CONFIG: Record<
  string,
  { shape: 'circle' | 'square' | 'triangle' | 'star' | 'pin'; label: string }
> = {
  // Santé
  ...
  public async initializeInstanceData(
    ...
    for (const bm of defaultBaseMaps) {
      try {
        ...
        logger.warn('Échec création fond de carte par défaut', {
          slug: bm.slug,
          instanceId,
          error: String(bmErr),
        });
    ...
    for (const cat of defaultCategories) {
      ...
      logger.debug('Sous-groupe par défaut créé pour le groupe', {
        subGroupSlug,
    ...
    for (const layerConfig of defaultLayers) {
      ...
      const svgConf = LAYER_SVG_CONFIG[layerConfig.slug] ?? {
        shape: 'circle' as const,
        label: layerConfig.slug.substring(0, 2).toUpperCase(),
      };
      ...
      try {
        ...
        logger.info('Table spatiale créée avec succès', {
          layerSlug,
          tableName: dbTableName,
          stats,
        });
