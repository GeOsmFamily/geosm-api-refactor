...
export class AdressageService {
  ...
  async getPoints(coord: [number, number], nomRue: string): Promise<PointResult[]> {
    ...
    const rows = await this.prisma.$queryRawUnsafe<(PointResult & { st_intersects: boolean; nom_voie: string })[]>(
    ...
    return rows.filter(r => r.st_intersects && r.nom_voie.replace(/\s+/g, '').toUpperCase() === normalizedRue);
  }
  ...
  async getElasticData(data: { shema: string; table: string; key_couche: string; id: number }[]): Promise<AdresseResult[]> {
