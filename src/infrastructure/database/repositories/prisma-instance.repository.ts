import {
  ...
  InstanceUser as PrismaInstanceUser,
  Prisma,
} from '@prisma/client';
import {
  IInstanceRepository,
...
export class PrismaInstanceRepository implements IInstanceRepository {
  ...
  async create(data: Omit<Instance, 'createdAt' | 'updatedAt'>): Promise<Instance> {
    const record = await this.prisma.instance.create({
      data: {
        ...
        boundaryTable: data.boundaryTable,
        boundaryId: data.boundaryId,
        boundaryGeomCol: data.boundaryGeomCol ?? undefined,
  ...
  async update(
    id: string,
    data: Partial<Omit<Instance, 'id' | 'createdAt' | 'updatedAt'>>,
  ): Promise<Instance> {
  ...
  async addInstanceUser(
    instanceId: string,
    userId: string,
    role: Role,
  ): Promise<InstanceUserRecord> {
  ...
  private toDomain(record: PrismaInstance): Instance {
    return new Instance({
      ...
      boundaryGeomCol:
        ((record as Record<string, unknown>).boundaryGeomCol as string | null) ?? null,
      adminLevel: ((record as Record<string, unknown>).adminLevel as number | null) ?? null,
      parentInstanceId:
        ((record as Record<string, unknown>).parentInstanceId as string | null) ?? null,
  ...
  private toInstanceUserRecord(
    record: PrismaInstanceUser & {
